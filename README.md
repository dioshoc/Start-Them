...
Если же вдруг у заказчика свой ***МЕГА СУПЕР АХЕРЕННЫЙ ШРИФТ*** и в других он души не видит, то на данный момент я могу только посочувствовать вам и отправить в гугл, так как я пытался найти как это подключить и за две минуты ничего не нашел, в данный момент времени нет что бы разобрться, но когда нибудь будет
...
Единственное что стоит упомянуть это следующие строчки кода, если произошли какие то изменения в function.php уже после запуска проекта, то увиличивпайте по чуть чуть версию, это надо для отчистки кэша php функций.
